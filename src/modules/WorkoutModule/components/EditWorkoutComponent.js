...
import style from './EditWorkoutComponent.css'
...
import { deleteWorkout, editWorkout } from '../reducers/WorkoutReducer'
import { ThinButton } from '../../../components/ThinButton'

const EditWorkoutComponent = (props) => {
  ...
  const workout = workoutsState.allMyWorkouts.find(el => el.id === parseInt(props.match.params.id))
  ...
  const [formData, setFormData] = useState({
    ...
    sets: workout.sets.map(el => { return { exercise: el.exercise_id, repetitions: el.repetitions, weight: el.pivot.weight, id: el.id } }),
    submitted: false
  })
 ...
 */
  ...
  return (
    <div className={style.main}>
      {formData.submitted && workoutsState.deleteWorkout.success && <Redirect to='/my-workouts'></Redirect>}
      {formData.submitted && workoutsState.editWorkout.success && <Redirect to='/my-workouts'></Redirect>}
      <h1 className={style.createWorkout__title}>Editar registro de ejercicio</h1>
      <div className={style.createWorkoutWrapper}>
        <div className={`${style.column} ${style.firstColumn}`}>
          ...
          <ThinButtonRed onClick={() => setShowDeleteModal(true)}className={style.deleteWorkout}>Eliminar este ejercicio</ThinButtonRed>
          </form>
        </div>
      ...
      {showDeleteModal && <div className={style.delete__modalWrapper}>
        <div className={style.delete__modal}>
          <p>¿Estás seguro de eliminar este registro? Esta acción es irreversible.</p>
          <div className={style.delete__modalControls}>
            ...
            <ThinButtonRed className={style.delete__modalControl} onClick={() => setShowDeleteModal(false)}>Cancelar</ThinButtonRed>
          </div>
        </div>
      </div>}
    </div>
